...
def build_f03_fixture(
    out_path: str | Path,
    system_owner: str | None = "李大華",
    ...
    for it in items or []:
        ...
        if state is not None:
            if state not in F03_CHECK_COLS:
                raise ValueError(f"未知勾選狀態 {state!r}，應為 {list(F03_CHECK_COLS)} 之一")
            ws[f"{F03_CHECK_COLS[state]}{r}"] = "V"
        if it.get("proposal") is not None:
