...
# 結構化輸出 schema：與 _USER_TEMPLATE 的輸出格式一致，供 client 以 json_schema（strict）模式約束回應。
# strict 要求每層物件都 additionalProperties=False，且所有欄位列入 required。
_VERDICT_DETAIL = {
    "difference": {"flag": "boolean", "summary": "string"},
    "proposal": {"vague": "boolean", "reason": "string"},
    "golive": {"vague": "boolean", "reason": "string"},
}
_VERDICT_SCHEMA = {
    "type": "object",
    "properties": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    **{
                        ...
                        for section, fields in _VERDICT_DETAIL.items()
                    },
                },
                "required": ["item_id", *_VERDICT_DETAIL],
                "additionalProperties": False,
            },
        },
    },
...
def _judge_batch(client: ChatClient, chunk: list[F03ChecklistItem]) -> dict[str, dict]:
    ...
    obj = parse_json_object(client.chat(messages, schema=_VERDICT_SCHEMA))
