...
def test_treatment_threshold_at_six_negative(tmp_path):
    # 剩餘風險分數恰為 6 即須續填處理計畫（依附件四：≥6，非 >6）
    f = build_f02_fixture(
        tmp_path / "treat6.xlsm", answers=HIGH_RISK,
        residual_rows=[["控管要求A", "部分", "說明", "中", 12, 6]],
    )
    assert "F02.TREATMENT_MISSING" in codes(f)


def test_treatment_threshold_below_six_positive(tmp_path):
    # 剩餘分數 5（<6）已填剩餘風險表但毋需處理計畫
    f = build_f02_fixture(
        tmp_path / "treat5.xlsm", answers=HIGH_RISK,
        residual_rows=[["控管要求A", "部分", "說明", "中", 12, 5]],
    )
    found = codes(f)
    assert "F02.RESIDUAL_MISSING" not in found
    assert "F02.TREATMENT_MISSING" not in found
...
# ---------- 報告風險欄位：固有風險分數/分級（有 ground truth 數值）----------
def test_report_risk_fields_high(tmp_path):
    # HIGH_RISK 答案集確定性重算 → overall 96.30%、分級「高」
    report = review_f02(build_f02_fixture(tmp_path / "risk_high.xlsm", answers=HIGH_RISK))
    assert report.risk_grade == "高"
    assert report.risk_score == pytest.approx(96.296, abs=0.01)


def test_report_risk_fields_low(tmp_path):
    # BASELINE 答案集確定性重算 → overall 18.75%、分級「低」
    report = review_f02(make(tmp_path, "risk_low"))
    assert report.risk_grade == "低"
    assert report.risk_score == pytest.approx(18.75, abs=0.01)
